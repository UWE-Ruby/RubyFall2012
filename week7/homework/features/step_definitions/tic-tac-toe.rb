...
class TicTacToe
  ...
  def determine_winner
      if @board[:A1] == @board[:A2] && @board[:A2] == @board[:A3] && @board[:A1] != nil
        victory = true
        winner_symbol = @board[:A1]
      elsif @board[:B1] == @board[:B2] && @board[:B2] == @board[:B3] && @board[:B1] != nil
        victory = true
        winner_symbol = @board[:B1]
      elsif @board[:C1] == @board[:C2] && @board[:C2] == @board[:C3] && @board[:C1] != nil
        victory = true
        winner_symbol = @board[:C1]
      elsif @board[:A1] == @board[:B1] && @board[:B1] == @board[:C1] && @board[:A1] != nil
        victory = true
        winner_symbol = @board[:A1]
      elsif @board[:A2] == @board[:B2] && @board[:B2] == @board[:C2] && @board[:A2] != nil
        victory = true
        winner_symbol = @board[:A2]
      elsif @board[:A3] == @board[:B3] && @board[:B3] == @board[:C3] && @board[:A3] != nil
        victory = true
        winner_symbol = @board[:A3]
      elsif @board[:A1] == @board[:B2] && @board[:B2] == @board[:C3] && @board[:A1] != nil
        victory = true
        winner_symbol = @board[:A1]
      elsif @board[:A3] == @board[:B2] && @board[:B2] == @board[:C1] && @board[:A3] != nil
        victory = true
        winner_symbol = @board[:A3]
      else
        victory = false
